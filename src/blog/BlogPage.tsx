...
const BlogCard = ({ post }: { post: BlogPost }) => (
  <article className="group overflow-hidden rounded-2xl bg-white shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
    ...
    <div className="p-6">
      ...
      <Link to={`/blog/${post.slug}`}>
        <h2 className="mb-2 text-xl font-bold text-amber-950 transition-colors group-hover:text-amber-700">
          {post.title}
        </h2>
      </Link>
      <p className="mb-5 text-sm leading-relaxed text-amber-800/70">
        {post.excerpt}
      </p>
      <div className="flex items-center justify-between">
        ...
        <Link
          to={`/blog/${post.slug}`}
          className="rounded-full bg-amber-950 px-4 py-2 text-xs font-semibold text-amber-50 transition-colors hover:bg-amber-700"
        >
          Lire l'article
        </Link>
      </div>
...
const BlogPage = () => (
  <main className="min-h-screen pb-0">
    <div className="bg-amber-950 pb-16 pt-28 text-center text-amber-50">
