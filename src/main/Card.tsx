...
const Card: React.FC = () => {
  ...
  if (error) return <p className="text-amber-950 dark:text-amber-100">Erreur: {error}</p>;
        ...
        {data.map((country) => (
          ...
          >
            <div className="relative h-64 w-full shadow-xl transition-all duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] sm:h-80">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center rounded-lg bg-amber-200 px-6 py-8 text-center text-lg text-slate-200 dark:bg-stone-700 [backface-visibility:hidden] [transform:rotateY(180deg)]">
                <h3 className="mb-2 text-lg font-bold text-amber-950 dark:text-amber-100">
                  {country.name.common}
                </h3>
                <p className="mb-2 text-lg text-amber-950 dark:text-amber-200">{country.region}</p>
